...
int main(int argc, char** argv)
{
    ...
    fprintf(stdout, "\nExtracting a ranked list of named entities from an "
                    "URL.\n");
    if (ALCHEMY_OK != alchemy_url_get_ranked_named_entities(
    ...
    fprintf(stdout, "\nExtracting a ranked list of named entities from a HTML "
                    "document.\n");
    if (ALCHEMY_OK != alchemy_html_get_ranked_named_entities(
                    &api_obj, html, "http://www.test.com/",
                    alchemy_stdout_callback, NULL))
        alchemy_error_exit("alchemy_html_get_ranked_named_entities invocation "
                         "failed", &api_obj, html);

    fprintf(stdout, "\nExtracting a ranked list of named entities from a text "
                    "string.\n");
    if (ALCHEMY_OK != alchemy_text_get_ranked_named_entities(&api_obj,
                    "Hello there, my name is Bob Jones.  I live in the United "
                    "States of America.  Where do you live, Fred?",
                    alchemy_stdout_callback, NULL))
        alchemy_error_exit("alchemy_text_get_ranked_named_entities invocation "
                         "failed", &api_obj, html);
