/*************************************************************************
 * Summary: AlchemyAPI Example - Topic Keyword Extraction
...
int main(int argc, char** argv)
{
    ...
    fprintf(stdout, "\nExtracting topic keywords from an URL.\n");
    if (ALCHEMY_OK != alchemy_url_get_ranked_keywords(&api_obj, "http://www.techcrunch.com/",
                                                  alchemy_stdout_callback, NULL))
        alchemy_error_exit("alchemy_url_get_ranked_keywords invocation failed",
                         &api_obj, html);

    fprintf(stdout, "\nExtracting topic keywords from a HTML document.\n");
    if (ALCHEMY_OK != alchemy_html_get_ranked_keywords(&api_obj, html, "http://www.test.com/",
                                                   alchemy_stdout_callback, NULL))
        alchemy_error_exit("alchemy_html_get_ranked_keywords invocation failed",
                         &api_obj, html);

    fprintf(stdout, "\nExtracting topic keywords from a text string.\n");
    if (ALCHEMY_OK != alchemy_text_get_ranked_keywords(&api_obj,
